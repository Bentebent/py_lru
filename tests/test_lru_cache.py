...
def test_add_and_get() -> None:
    ...
    assert cache.get_the_entry("key1", None) == "value1"
...
def test_overwrite_value() -> None:
    ...
    assert cache.get_the_entry("key1", None) == "value2"
...
def test_capacity_eviction(cache_with_removal_log: LRUCache, removal_log: List[Any]) -> None:
    ...
    assert cache.get_the_entry("key1", None) is None
...
def test_clear(cache_with_removal_log: LRUCache, removal_log: List[Any]) -> None:
    ...
    cache.clear()
    assert cache.get_the_entry("key1", None) is None
    assert cache.get_the_entry("key2", None) is None
...
def test_lru_order() -> None:
    ...
    cache.add("key1", "value1")
    cache.add("key2", "value2")
    cache.get_the_entry("key1", None)  # Access key1 to move it to the front
    cache.add("key3", "value3")  # This should evict key2
    assert cache.get_the_entry("key2", None) is None
    assert cache.get_the_entry("key1", None) == "value1"
    assert cache.get_the_entry("key3", None) == "value3"
